...
export class GestionPreguntaDialogComponent implements OnChanges {
  ...
  cargarPreguntaParaEdicion(pregunta: any) {
    ...
    this.tipoPregunta = pregunta.tipo || 'ABIERTA';
  ...
  resetForm() {
    this.textoPregunta = '';
    this.tipoPregunta = 'ABIERTA';
    this.opciones = [];
    this.numeroPregunta = null;
  }

  agregarOpcion() {
  this.opciones.push({ texto: '', numero: this.opciones.length + 1 });
}


  eliminarOpcion(index: number) {
  this.opciones.splice(index, 1);
  this.opciones.forEach((op, i) => op.numero = i + 1);
}
  ...
  guardarPregunta() {
    ...
    if ((this.tipoPregunta === 'OPCION_MULTIPLE_SELECCION_SIMPLE' || this.tipoPregunta === 'OPCION_MULTIPLE_SELECCION_MULTIPLE') && this.opciones.length === 0) {
      this.showError('Agregá al menos una opción.');
      return; 
    }

    if ((this.tipoPregunta === 'OPCION_MULTIPLE_SELECCION_SIMPLE' || this.tipoPregunta === 'OPCION_MULTIPLE_SELECCION_MULTIPLE')) {
    ...
    if ((this.tipoPregunta === 'OPCION_MULTIPLE_SELECCION_SIMPLE' || this.tipoPregunta === 'OPCION_MULTIPLE_SELECCION_MULTIPLE') && this.opciones.length < 2) {
