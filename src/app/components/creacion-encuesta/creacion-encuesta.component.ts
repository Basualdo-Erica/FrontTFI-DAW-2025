...
export class CreacionEncuestaComponent {
  ...
  switch(tipo) {
    ...
    case 'OPCION_SIMPLE': return 'Opción Simple';
    ...
    this.encuestasService.crearEncuesta(encuesta).subscribe({
      next: (res) => {
        ...
        this.router.navigateByUrl(
          `/presentacion-enlaces?id-encuesta=${res.id}&codigo-respuesta=${res.codigoRespuesta}&codigo-resultados=${res.codigoResultados}`
        );
      },
      error: () => {
        this.messageService.add({
